...
- Cho phép agent dịch và phân tích tự chạy, còn người dùng chỉ chỉnh dữ liệu khi cần.
- Cho phép sửa bản dịch, glossary, alias và raw text trực tiếp trên UI rồi ghi DB ngay.
...
- Review queue không được là cổng bắt buộc làm pipeline dừng lại.
- Chuỗi UI và prompt dịch phải lấy từ registry, không hardcode trong code tính năng.
- Sửa bản dịch hoặc glossary phải đánh dấu stale cho dữ liệu phụ thuộc.
